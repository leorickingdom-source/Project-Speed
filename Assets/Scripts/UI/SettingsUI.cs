...
public static class SettingsUI
{
    ...
    public const float Height = 254f;
    ...
    static float Row(float x, float y, float w, string name, float v, float min, float max,
        string fmt)
    {
        GUI.Label(new Rect(x, y, w * 0.6f, 24f), name, label);
        GUI.Label(new Rect(x + w * 0.6f, y, w * 0.4f, 24f), v.ToString(fmt), value);
        return GUI.HorizontalSlider(new Rect(x, y + 24f, w, 18f), v, min, max);
    }
    ...
    public static bool Draw(float x, float y, float w, string title = "SETTINGS")
    {
        ...
        {
            GUI.Label(new Rect(x, y, w, 24f), title, label);
            y += 26f;
        }
        ...
        GUI.Label(new Rect(x, y, w * 0.45f, 24f), "Player name", label);
        string typed = GUI.TextField(new Rect(x + w * 0.45f, y - 2f, w * 0.55f, 28f),
        ...
        bool openControls = GUI.Button(new Rect(x, y, half, 32f), "Controls…", button);
        bool reset = GUI.Button(new Rect(x + half + 8f, y, half, 32f), "Reset to default", button);
